...
Haffman::Haffman(string fileName)
{
    ...
    string outputFileName = fileName.substr(0, fileName.length() - 4) + "_pack.exe";
    this->outputFile.open(outputFileName.c_str(), ios::out | ios::binary);
    if (!outputFile)
    {
        this->status = false;
        return;
...
/// Write Unpackager into output file.
void Haffman::WriteUnpackager()
{
